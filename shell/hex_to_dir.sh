...
#!/usr/bin/env bash
echo "This script will parse prilog.txt and do actions written in comment at the script begining"

INPUT_PRILOG=$1
HEX_VALUE=()
for i in $(cat $INPUT_PRILOG)
do
  if [ `uname -s` = "Darwin" ]; then # radim na meku pa mi je ovo bilo potrebno zbog line ending-a u fajlu
    file=`echo $i | tr '\r' '\n'`
    touch $file
  else
    touch $i
  fi
  HEX_CALC=`echo $i | sed 's/k//' | awk -F"." '{print $1"\n"}'` # izdvajanje samo hex dela
  if [[ $HEX_CALC =~ ^[A-Fa-f0-9]+$ ]]; then # provera da li je u dobrom formatu
   HEX_VALUE+=( "$HEX_CALC" )
  else
    echo "Invalid file"
    echo `pwd`"/$i" # ispis losih fajlova
  fi
done

for j in "${HEX_VALUE[@]}" # prolaz kroz listu dobrih fajlova
do
  STR_LENGTH=`echo $j | awk '{print length}'`
  if [ $STR_LENGTH == 8 ]; then
    G=`echo ${j:6:1}`
    E=`echo ${j:4:1}`
    if [[ $G =~ ^[0-9] ]]; then # provera da li je G broj
      if [[ $((G%2)) -eq 0 ]]; then # provera da li je paran ili neparan
        mkdir -p ${G}0/${E}0
        mv k${j}.kod ${G}0/${E}0
      else
        X=`echo ${G}-1 | bc`
